<form id="insertpreference" method="POST" action="{{ route('users.editprofile.partner.update', $userid) }}" class="padding-0-30 show-form"
style="display: block;">
@csrf
...
<div class="form-group row">
    ...
    <div class="col-md-8">
        <div class="radio">
            <label>
                <input type="radio" class="diet" name="diet" value="1">
                <span>Yes</span></label>
            <label>
                <input type="radio" class="diet" name="diet" value="2">
                <span>No</span></label>
        </div>
        @if ($errors->has('diet'))
        <span class="text-danger" role="alert">
...
<div class="form-group row">
    ...
    <div class="col-md-8">
        <select name="livingcountry" id="country" class="form-control" required="required">
            <option>Select ... </option>
            <option value="0">Don't have Prefernece</option>
            @foreach ($parterCountry as $key => $value)
